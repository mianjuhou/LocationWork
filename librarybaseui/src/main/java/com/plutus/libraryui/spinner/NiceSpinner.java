...
public class NiceSpinner extends TextView {
	...
	private BaseNiceSpinnerAdapter mAdapter;
	 ...
	 * @param index
	 */
	public void perfromOnItemClick(int index) {
		View childAt = mListView.getChildAt(index);
		mListView.performItemClick(childAt, index, index);
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				// if (position >= mSelectedIndex && position <
				// mAdapter.getCount()) {
				// position++;
				// }
		...
		if (!mHideArrow) {
			Drawable basicDrawable = ContextCompat.getDrawable(context, R.drawable.spiner_arrow);
	...
	public int getSelectedIndex(String text) {
		if (mAdapter != null) {
			...
			for (int i = 0; i < count; i++) {
				String string = mAdapter.getItemInDataset(i).toString();
	...
	public void setSelectedIndex(int position) {
		...
		if (mAdapter != null) {
			if (position >= 0 && position <= mAdapter.getCount()) {
				...
				// throw new IllegalArgumentException("Position must be lower
				// than adapter count!");
			}
		}
	}
	...
	private void setAdapter(@NonNull ListAdapter adapter) {
		mAdapter = new NiceSpinnerAdapterWrapper(getContext(), adapter);
		setAdapterInternal(mAdapter);
	}

	public void setAdapterInternal(@NonNull BaseNiceSpinnerAdapter adapter) {
		mAdapter = adapter;
		mSelectedIndex = 0;// 默认第一条可见
