/*
 * Copyright (c) 1997, 2006, Oracle and/or its affiliates. All rights reserved. ORACLE
 ...
 * examples of using default mutable tree nodes, see
 * <a href="http://java.sun.com/docs/books/tutorial/uiswing/components/tree.html">How to Use
 * Trees</a> in <em>The Java Tutorial.</em>
 *
 * <p>
 *
 * A tree node may have at most one parent and 0 or more children.
 * <code>DefaultMutableTreeNode</code> provides operations for examining and modifying a node's
 * parent and children and also operations for examining the tree that the node is a part of. A
 * node's tree is the set of all nodes that can be reached by starting at the node and following all
 * the possible links to parents and children. A node with no parent is the root of its tree; a node
 * with no children is a leaf. A tree may consist of many subtrees, each node acting as the root for
 * its own subtree.
 * <p>
 * This class provides enumerations for efficiently traversing a tree or subtree in various orders
 * or for following the path between two nodes. A <code>DefaultMutableTreeNode</code> may also hold
 * a reference to a user object, the use of which is left to the user. Asking a
 * <code>DefaultMutableTreeNode</code> for its string representation with <code>toString()</code>
 * returns the string representation of its user object.
 * <p>
 * <b>This is not a thread safe class.</b>If you intend to use a DefaultMutableTreeNode (or a tree
 * of TreeNodes) in more than one thread, you need to do your own synchronizing. A good convention
 * to adopt is synchronizing on the root node of a tree.
 * <p>
 * While DefaultMutableTreeNode implements the MutableTreeNode interface and will allow you to add
 * in any implementation of MutableTreeNode not all of the methods in DefaultMutableTreeNode will be
 * applicable to all MutableTreeNodes implementations. Especially with some of the enumerations that
 * are provided, using some of these methods assumes the DefaultMutableTreeNode contains only
 * DefaultMutableNode instances. All of the TreeNode/MutableTreeNode methods will behave as defined
 * no matter what implementations are added.
 *
 * <p>
...
public class DefaultMutableTreeNode implements Cloneable, MutableTreeNode, Serializable {
	...
	/**
	 * An enumeration that is always empty. This is used when an enumeration of a leaf node's
	 ...
	 * <code>childIndex</code>. <code>newChild</code> must not be null and must not be an ancestor
	 * of this node.
	 *
	 * @param newChild
	...
	/**
	 * Removes the child at the specified index from this node's children and sets that node's
	 * parent to null. The child node to remove must be a <code>MutableTreeNode</code>.
	 *
	 * @param childIndex
	 ...
	 * not a child of this node, returns <code>-1</code>. This method performs a linear search and
	...
	/**
	 * Creates and returns a forward-order enumeration of this node's children. Modifying this
	 * node's child array invalidates any child enumerations created before the modification.
	 *
	 ...
	 * is considered a descendant of itself. If <code>anotherNode</code> is null, returns false.
	 ...
	 * such ancestor exists -- if this node and <code>aNode</code> are in different trees or if
	 * <code>aNode</code> is null. A node is considered an ancestor of itself.
	 *
	 * @see #isNodeAncestor
	 * @see #isNodeDescendant
	 * @param aNode
	...
	/**
	 * Returns true if and only if <code>aNode</code> is in the same tree as this node. Returns
	 ...
	 * @param depth
	 *            an int giving the number of steps already taken towards the root (on recursive
	 ...
	 * null if this node is the last node of the traversal. This is an inefficient way to traverse
	 ...
	 * <code>null</code> if this node is the first node of the traversal -- the root of the tree.
	 * This is an inefficient way to traverse the entire tree; use an enumeration, instead.
	 *
	 * @see #preorderEnumeration
	 ...
	 * preorder. The first node returned by the enumeration's <code>nextElement()</code> method is
	 * this node.
	 * <P>
	 *
	...
	/**
	 * Creates and returns an enumeration that traverses the subtree rooted at this node in
	 * postorder. The first node returned by the enumeration's <code>nextElement()</code> method is
	 ...
	 * between this node and <code>ancestor</code>, inclusive. Each <code>nextElement()</code>
	 * message is O(1).
	 * <P>
	 *
	 * Modifying the tree by inserting, removing, or moving a node invalidates any enumerations
	...
	/**
	 * Returns true if <code>aNode</code> is a child of this node. If <code>aNode</code> is null,
	 * this method returns false.
	 *
	 * @return true if <code>aNode</code> is a child of this node; false if <code>aNode</code> is
	 *         null
	 */
	 ...
	 * A node is its own sibling. If <code>anotherNode</code> is null, returns false.
	 *
	 * @param anotherNode
	...
	/**
	 * Returns the number of siblings of this node. A node is its own sibling (if it has no parent
	 * or no siblings, this method returns <code>1</code>).
	 *
	 ...
	 * node has no parent or is the parent's last child. This method performs a linear search that
	 * is O(n) where n is the number of children; to traverse the entire array, use the parent's
	 * child enumeration instead.
	 *
	 * @see #children
	...
	/**
	 * Returns the previous sibling of this node in the parent's children array. Returns null if
	 * this node has no parent or is the parent's first child. This method performs a linear search
	 * that is O(n) where n is the number of children.
	 *
	 ...
	 * and nodes that <i>cannot</i> have children (e.g. to distinguish files from empty
	 * directories), use this method in conjunction with <code>getAllowsChildren</code>
	 *
	 ...
	 * returns <code>1</code>. This method is O(n) where n is the number of descendants of this
	 * node.
	 *
	 * @see #isNodeAncestor
	...
	/**
	 * Returns the result of sending <code>toString()</code> to this node's user object, or the
	 * empty string if the node has no user object.
	 *
	 * @see #getUserObject
	 */
