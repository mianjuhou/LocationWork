package com.jtv.location.libraryface.activity;

import java.util.List;
import java.util.concurrent.locks.Lock;
import java.util.concurrent.locks.ReentrantLock;

import com.jtv.location.libraryface.R;
...
public class MainActivity extends Activity implements OnClickListener {
	...
	protected ProcessImageAndDrawResults mDraw;
	protected final String database = "Memory50.dat";
	...
	public FSDKlistener listenr;// 监听状态
	protected boolean allowModify;// 是否允许修改人员

	public void showErrorAndClose(String error, int code) {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setMessage(error + ": " + code).setPositiveButton("确定", new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialogInterface, int i) {
				onClickError(dialogInterface, i);
			}
		}).show();
	}
	...
	public String getLisenseCode() {
		return "uKTAEoswrmpDk4M1aTuxTEzPltWjuCdpJH8RiK+kNKdjr0uGHVtoA3mHPuBDM+6KZeOINo2UwdXpXmoiF9FVx2hlrJQhTDUSrD4Dwajvb8B8ttUtqk0l9tho1due6fi7n/p9s1ZhGL1XxUNsRlNT4edDpx1TATwUi7PU29woF10=";
	}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (this instanceof FSDKlistener) {
			listenr = (FSDKlistener) this;
		}

		int res = FSDK.ActivateLibrary(getLisenseCode());
		if (res != FSDK.FSDKE_OK) {
			mIsFailed = true;
			showErrorAndClose("打开失败,返回错误状态码", res);
		} else {
			FSDK.Initialize();
			...
			if (FSDK.FSDKE_OK != FSDK.LoadTrackerMemoryFromFile(mDraw.mTracker, getDateBaseAddres())) {
			...
			FSDK.SetTrackerMultipleParameters(mDraw.mTracker,
					"ContinuousVideoFeed=true;RecognitionPrecision=0;Threshold=0.997;Threshold2=0.9995;ThresholdFeed=0.97;MemoryLimit=1000;HandleArbitraryRotations=false;DetermineFaceRotationAngle=false;InternalResizeWidth=70;FaceDetectionThreshold=5;",
					errpos);
			if (errpos[0] != 0) {
				showErrorAndClose("Error setting tracker parameters, position", errpos[0]);
			}

			this.getWindow().setBackgroundDrawable(new ColorDrawable()); // black
																			// background
	...
	public void onClick(View view) {
		if (view.getId() == R.id.helpButton) {
			...
			FSDK.LockID(mDraw.mTracker, mDraw.lastIndex);
			FSDK.SetName(mDraw.mTracker, mDraw.lastIndex, "");
			FSDK.LockID(mDraw.mTracker, mDraw.lastIndex);
			Toast.makeText(this, new Long(mDraw.lastIndex).toString(), Toast.LENGTH_SHORT).show();
	...
	public void onPause() {
		super.onPause();
		pauseProcessingFrames();
		FSDK.SaveTrackerMemoryToFile(mDraw.mTracker, getDateBaseAddres());
	}
	...
	private void pauseProcessingFrames() {
		...
		// It is essential to limit wait time, because mStopped will not be set
		// to 0, if no frames are feeded to mDraw
		for (int i = 0; i < 100; ++i) {
			if (mDraw.mStopped != 0)
				break;
			try {
...
class ProcessImageAndDrawResults extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		ImageUtilEngine.decodeYUV420SP2RGB24(mRGBData, mYUVData, mImageWidth, mImageHeight);
		...
		// it is necessary to work with local variables (onDraw called not the
		// time when mImageWidth,... being reassigned, so swapping mImageWidth
		// and mImageHeight may be not safe)
		...
		/*
		 * if (!first_frame_saved) { first_frame_saved = true; String
		 * galleryPath =
		 * Environment.getExternalStoragePublicDirectory(Environment.
		 * DIRECTORY_PICTURES).getAbsolutePath();
		 * FSDK.SaveImageToFile(RotatedImage, galleryPath + "/first_frame.jpg");
		 * //frame is rotated! }
		 */
		...
		for (int i = 0; i < face_count[0]; ++i) {
			canvas.drawRect(mFacePositions[i].x1, mFacePositions[i].y1, mFacePositions[i].x2, mFacePositions[i].y2,
			...
			if (IDs[i] != -1) {
				...
				if (names[0] != null && names[0].length() > 0) {
					canvas.drawText(names[0], (mFacePositions[i].x1 + mFacePositions[i].x2) / 2,
							mFacePositions[i].y2 + shift, mPaintBlue);
					named = true;
					if (activity != null && activity.listenr != null) {
						activity.listenr.successfule(names[0]);
					}
				}
			}
			if (!named) {
				if (activity != null && activity.listenr != null) {
					activity.listenr.failed();
				}
				if (activity != null && activity.allowModify) {
					canvas.drawText("按下我输入名字", (mFacePositions[i].x1 + mFacePositions[i].x2) / 2,
							mFacePositions[i].y2 + shift, mPaintGreen);
				} else {
					canvas.drawText("此人信息不存在", (mFacePositions[i].x1 + mFacePositions[i].x2) / 2,
							mFacePositions[i].y2 + shift, mPaintGreen);
				}
	...
	@Override
	public boolean onTouchEvent(MotionEvent event) { // NOTE: the method can be
														// implemented in
														// Preview class
		if (activity != null && !activity.allowModify) {
			return true;
		...
		case MotionEvent.ACTION_DOWN:
			...
			for (int i = 0; i < MAX_FACES; ++i) {
				if (rects[i] != null && rects[i].x1 <= x && x <= rects[i].x2 && rects[i].y1 <= y
						&& y <= rects[i].y2 + 30) {
					mTouchedID = IDs[i];
...
class Preview extends SurfaceView implements SurfaceHolder.Callback {
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
			builder.setMessage("Cannot open camera").setPositiveButton("Ok", new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialogInterface, int i) {
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
		...
		for (Size s : supportedSizes) {
			if ((width - 640) * (width - 640) + (height - 480) * (height - 480) > (s.width - 640) * (s.width - 640)
					+ (s.height - 480) * (s.height - 480)) {
				width = s.width;
				height = s.height;
			}
		}
