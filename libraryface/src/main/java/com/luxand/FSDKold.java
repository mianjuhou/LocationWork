...
public class FSDKold {
	static {

		try {
			
			System.loadLibrary("stlport_shared");
			boolean loadDefaultSO = FaceUtil.isLoadDefaultSO();
			
			if (loadDefaultSO) {

				System.loadLibrary("fsdk"); // 动态加载so的体积减少app的体积
			...
			try {
				String dynamicSo = FaceUtil.getDynamicSo();
				System.load(dynamicSo);// 动态加载sdk路径不支持x86会报错，需要放x86so文件在
	...
	public static class HImage { // to pass image "by reference"
	...
	// public static native int GetHardware_ID(String HardwareID[]); //not
	// implemented
	...
	public static native int LoadImageFromBuffer(HImage Image, byte Buffer[], int Width, int Height, int ScanLine,
	...
	public static native int SetFaceDetectionParameters(boolean HandleArbitraryRotations,
			boolean DetermineFaceRotationAngle, int InternalResizeWidth);
	...
	public static native int DetectFacialFeaturesInRegion(HImage Image, TFacePosition FacePosition,
	...
	public static native int RotateImageCenter(HImage SourceImage, double angle, double xCenter, double yCenter,
			HImage DestImage);

	public static native int CopyRect(HImage SourceImage, int x1, int y1, int x2, int y2, HImage DestImage);

	public static native int CopyRectReplicateBorder(HImage SourceImage, int x1, int y1, int x2, int y2,
			HImage DestImage);

	public static native int MirrorImage(HImage Image, boolean UseVerticalMirroringInsteadOfHorizontal);

	public static native int ExtractFaceImage(HImage Image, FSDK_Features FacialFeatures, int Width, int Height,
			HImage ExtractedFaceImage, FSDK_Features ResizedFeatures);

	public static native int GetFaceTemplate(HImage Image, FSDK_FaceTemplate FaceTemplate);

	public static native int GetFaceTemplateInRegion(HImage Image, TFacePosition FacePosition,
			FSDK_FaceTemplate FaceTemplate);

	public static native int GetFaceTemplateUsingFeatures(HImage Image, FSDK_Features FacialFeatures,
			FSDK_FaceTemplate FaceTemplate);

	public static native int GetFaceTemplateUsingEyes(HImage Image, FSDK_Features EyeCoords,
			FSDK_FaceTemplate FaceTemplate);
	...
	public static native int GetTrackerParameter(HTracker Tracker, String ParameterName, String ParameterValue[],
			int MaxSizeInBytes);
	...
	public static native int GetTrackerFacialFeatures(HTracker Tracker, long CameraIdx, long ID,
			FSDK_Features FacialFeatures);

	public static native int GetTrackerFacePosition(HTracker Tracker, long CameraIdx, long ID,
	...
	public static native int GetTrackerFacialAttribute(HTracker Tracker, long CameraIdx, long ID, String AttributeName,
			String AttributeValues[], long MaxSizeInBytes);

	public static native int DetectFacialAttributeUsingFeatures(HImage Image, FSDK_Features FacialFeatures,
			String AttributeName, String AttributeValues[], long MaxSizeInBytes);
